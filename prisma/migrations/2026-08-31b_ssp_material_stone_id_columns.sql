-- Extends the SSP link (ssp_code/ssp_item_id, added 2026-08-31) down to the
-- material and stone rows, so an update to an already-linked item can
-- target SSP's existing material/stone rows instead of appending new ones.
ALTER TABLE samples
  ADD COLUMN IF NOT EXISTS ssp_material_id integer,
  ADD COLUMN IF NOT EXISTS ssp_stone_ids integer[];
